...
export price="50"
# echo ""
echo "Selling the product name $product "
echo ""
...
echo "Pending sells: "
sleep 10;
icad q nameservice list-pending-sell -o json |  jq '.pendingSell[] | select(.name=="amazon.org")'
...
# icad tx controller submit-tx ~/cosmos/sbip/scripts/message.json connection-0 --from $WALLET_1 --chain-id test-1 --home ~/cosmos/sbip/data/test-1 --node tcp://localhost:16657 --keyring-backend test -y  
