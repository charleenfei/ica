...
async  function workflow1() {
  const req = await fetch("/api/workflow4");
...
async  function workflow2() {
  const req = await fetch("/api/workflow4_buy");
  const data = await req.json();
  console.log(data);
...
async  function workflow1_check() {
  const req = await fetch("/api/workflow4_check");
  const data = await req.json();
  console.log(data);
...
export default function Home() {
  return (
    <div className={styles.container}>
      ...
      <div>
      <div className="d-grid gap-2 d-md-flex justify-content-md-end">
        <Button variant="primary" size="sm" onClick={workflow1}> Sell </Button>
        <Button variant="primary" size="sm" onClick={workflow2}> Buy </Button>
      ...
      <main>
        ...
        Workflow4: Match buyer and seller
        </div>
        <div className={styles.grid}>

        <div className={styles.card1}>
            <h5> Sell Transaction &rarr;</h5>
