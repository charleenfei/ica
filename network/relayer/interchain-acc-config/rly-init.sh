...
MNEMONIC_3="surround hip section canvas rocket misery vast mom stereo renew file arctic draw very feel say surge wide crucial parrot jacket wreck focus decade"
...
$BINARY paths add $CHAINID_3 $CHAINID_2 test3-test2 --file ./network/relayer/interchain-acc-config/paths/test3-test2.json --home $CHAIN_DIR/$RELAYER_DIR
...
$BINARY keys restore $CHAINID_3 testkey "$MNEMONIC_3" --home $CHAIN_DIR/$RELAYER_DIR

echo "Creating clients and a connection..."
$BINARY tx connection test1-test2 --home $CHAIN_DIR/$RELAYER_DIR
$BINARY tx connection test3-test2 --home $CHAIN_DIR/$RELAYER_DIR
